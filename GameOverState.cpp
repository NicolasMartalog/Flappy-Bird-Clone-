...
namespace Sonar
{
	...
	{
	    // importing the highscore from a txt file
        ...
        readFile.close();
        // making the txt file remeber the highscore
        ...
        // adding all the sprites I am going to need
		...
		_retryButton.setPosition(sf::Vector2f((_data->window.getSize().x / 2) - (_retryButton.getGlobalBounds().width / 2), _gameOverContainer.getPosition().y + _gameOverContainer.getGlobalBounds().height + (_retryButton.getGlobalBounds().height * 0.2)));
         // setting the text for the game over screen
...
// settign the medals to give out only until the conditiona are meet
	...
	void GameOverState::HandleInput()
	{
		...
		while (this->_data->window.pollEvent(event))
		{
            ...
            // restarts game if screen is pressed
